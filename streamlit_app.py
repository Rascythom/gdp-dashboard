...
# Přidání analýzy podle typu kurzu
uspesnost_nizky_kurz = 0
uspesnost_stredni_kurz = 0
uspesnost_vysoky_kurz = 0
pocet_nizky_kurz = 0
pocet_stredni_kurz = 0
pocet_vysoky_kurz = 0
...
if st.session_state.tikety:
    ...
    # Analýza úspěšnosti podle typu kurzu
    nizke_kurzy = df[df["kurz"] <= 2.0]
    stredni_kurzy = df[(df["kurz"] > 2.0) & (df["kurz"] <= 3.0)]
    vysoke_kurzy = df[df["kurz"] > 3.0]

    # Počítáme úspěšnost pro každý typ kurzu
    if not nizke_kurzy.empty:
        uspesnost_nizky_kurz = nizke_kurzy["výhra"].sum() / nizke_kurzy["castka"].sum() * 100
        pocet_nizky_kurz = nizke_kurzy.shape[0]
    if not stredni_kurzy.empty:
        uspesnost_stredni_kurz = stredni_kurzy["výhra"].sum() / stredni_kurzy["castka"].sum() * 100
        pocet_stredni_kurz = stredni_kurzy.shape[0]
    if not vysoke_kurzy.empty:
        uspesnost_vysoky_kurz = vysoke_kurzy["výhra"].sum() / vysoke_kurzy["castka"].sum() * 100
        pocet_vysoky_kurz = vysoke_kurzy.shape[0]
...
if pocet_nizky_kurz > 0:
    st.markdown(f"Úspěšnost při nízkých kurzech (do 2.0): {uspesnost_nizky_kurz:.2f}%")
if pocet_stredni_kurz > 0:
    st.markdown(f"Úspěšnost při středních kurzech (2.0–3.0): {uspesnost_stredni_kurz:.2f}%")
if pocet_vysoky_kurz > 0:
    st.markdown(f"Úspěšnost při vysokých kurzech (nad 3.0): {uspesnost_vysoky_kurz:.2f}%")
