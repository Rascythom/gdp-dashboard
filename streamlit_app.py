...
# Funkce pro nahrání a uložení tiketů
def load_tikety():
...
# Funkce pro nahrání a uložení uživatelů
def load_users():
...
def register_user(username, password):
    ...
    if username in users:
        st.warning("Toto uživatelské jméno již existuje.")
        return False
...
if st.session_state.user is None:
    ...
    if st.button("Registrovat"):
        if password != confirm_password:
            st.warning("Hesla se neshodují.")
        elif username and password:
            if register_user(username, password):
                st.success(f"Uživatel {username} byl úspěšně zaregistrován a přihlášen.")
        else:
            st.warning("Vyplňte všechna pole.")

    st.header("Přihlášení")
    login_username = st.text_input("Uživatelské jméno (pro přihlášení)")
    login_password = st.text_input("Heslo (pro přihlášení)", type="password")

    if st.button("Přihlásit se"):
        if login_user(login_username, login_password):
            st.success(f"Uživatel {login_username} byl úspěšně přihlášen.")
        else:
            st.warning("Neplatné přihlašovací údaje.")
else:
    st.header(f"Vítejte, {st.session_state.user}!")
